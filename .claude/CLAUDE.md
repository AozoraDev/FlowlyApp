...
- 业务模型：项目 sections → 明细 items；收支汇总走服务端 RPC 聚合
...
`.env`，`EXPO_PUBLIC_*` 前缀自动注入 process.env（公开，勿放密钥）：
...
- **路由**：`_layout.tsx` 引入 global.css；Provider 链 I18next→Query→Toast→Theme；NavBar + Stack + BottomBar，`headerShown:false`；typedRoutes 路径类型检查
...
- **Supabase**：单例客户端（Web localStorage / 移动端 AsyncStorage）；查询响应在 types.ts 边界 Zod 解析，禁 `as`/`!`；RLS 按 uid 隔离；分页 `range`+`count:'exact'`；汇总走 RPC `get_section_summary`/`get_section_summaries`；登录/OTP/会话见 auth.ts
